...
import { getActivityType, updateActivityType } from "./activityTypeSlice";
...
const editActivityType = ()=>{
  ...
  useEffect(()=>{
    dispatch(getActivityType(params.id));
  },[])
  
  const schema = yup
    .object({
      activityType: yup.string().required().min(2),
    })
    .required();
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
        ...
        activityType: event.target[1].value,
        isActive: event.target[2].value,
        id: params.id
       } ;
       
       dispatch(updateActivityType(formData));
  ...
  return (
    <>
      <div className="flex items-center mt-8 intro-y">
        <h2 className="mr-auto text-lg font-medium">Manage Activity Type</h2>
      </div>
                    ...
                    <form className="validate-form" onSubmit={onSubmit}>
                      ...
                      <FormInput
                        ...
                        <FormInput
                          ...
                          name="activityType"
                          className={clsx({
                            "border-danger": errors.activityType,
                          })}
                          placeholder="Activity Type"
                        />
                        {errors.activityType && (
                          <div className="mt-2 text-danger">
                            {typeof errors.activityType.message === "string" &&
