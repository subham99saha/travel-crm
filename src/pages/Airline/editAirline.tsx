...
import { getAirline, updateAirline } from "./AirlineSlice";

const editAirline = ()=>{
  const params = useParams();
  const dispatch = useAppDispatch();
  const { data } = useAppSelector((state) => state.airline); 

  useEffect(()=>{
    dispatch(getAirline(params.id));
  },[])
  
  const schema = yup
    .object({
      airlineName: yup.string().required().min(2),
    })
    .required();
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
        ...
        airlineName: event.target[1].value,
        isActive: event.target[2].value,
        id: params.id
       } ;
       
       dispatch(updateAirline(formData));
  ...
  return (
    <>
      <div className="flex items-center mt-8 intro-y">
        <h2 className="mr-auto text-lg font-medium">Manage Airline</h2>
      </div>
                    ...
                    <form className="validate-form" onSubmit={onSubmit}>
                      ...
                      <FormInput
                        ...
                        <FormInput
                          ...
                          name="airlineName"
                          className={clsx({
                            "border-danger": errors.airlineName,
                          })}
                          placeholder="Airline Name"
                        />
                        {errors.airlineName && (
                          <div className="mt-2 text-danger">
                            {typeof errors.airlineName.message === "string" &&
