...
import { getTravelType, updateTravelType } from "./TravelTypeSlice";

const editTravelType = ()=>{
  const params = useParams();
  const dispatch = useAppDispatch();
  const { data } = useAppSelector((state) => state.travelType); 

  useEffect(()=>{
    dispatch(getTravelType(params.id));
  },[])
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
        ...
        travelType: event.target[1].value,
        isActive: event.target[2].value,
        id: params.id
       } ;
       
       dispatch(updateTravelType(formData));
  ...
  return (
    <>
      <div className="flex items-center mt-8 intro-y">
        <h2 className="mr-auto text-lg font-medium">Manage Travel Type</h2>
      </div>
                    ...
                    <form className="validate-form" onSubmit={onSubmit}>
                      ...
                      <FormInput
                        ...
                        <FormInput
                          ...
                          name="travelType"
                          className={clsx({
                            "border-danger": errors.travelType,
                          })}
                          placeholder="Travel Type"
                        />
                        {errors.travelType && (
                          <div className="mt-2 text-danger">
                            {typeof errors.travelType.message === "string" &&
