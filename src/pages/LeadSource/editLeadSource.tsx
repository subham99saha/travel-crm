...
import { getLeadSource, updateLeadSource } from "./LeadSourceSlice";
...
const editLeadSource = ()=>{
  ...
  useEffect(()=>{
    dispatch(getLeadSource(params.id));
  },[])
  
  const schema = yup
    .object({
      leadType: yup.string().required().min(2),
    })
    .required();
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
        ...
        leadType: event.target[1].value,
        isActive: event.target[2].value,
        id: params.id
       } ;
       
       dispatch(updateLeadSource(formData));
                    ...
                    <form className="validate-form" onSubmit={onSubmit}>
                      ...
                      <FormInput
                        ...
                        <FormInput
                          ...
                          name="leadType"
                          className={clsx({
                            "border-danger": errors.leadType,
                          })}
                          placeholder="Currency"
                        />
                        {errors.leadType && (
                          <div className="mt-2 text-danger">
                            {typeof errors.leadType.message === "string" &&
