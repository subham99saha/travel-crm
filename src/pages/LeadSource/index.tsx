...
import { fetchAllLeadSource, deleteLeadSource } from "./LeadSourceSlice";
...
const Main = ()=>{
    ...
    const { data } = useAppSelector((state) => state.leadSource);

    useEffect(()=>{
      dispatch(fetchAllLeadSource());
  },[]);   
  ...
  const handleDelete = (id: any) => {
    dispatch(deleteLeadSource(deleteValueRef.current));
    dispatch(fetchAllLeadSource());
  ...
  return (
    <>
      <h2 className="mt-10 text-lg font-medium intro-y">Manage Lead Source</h2>
      <div className="grid grid-cols-12 gap-6 mt-5">
        <div className="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap">
        <Link to="/add-lead-source">
                ...
                <Table.Tr key={Key} className="intro-x">
                  ...
                  <Table.Td className="first:rounded-l-md last:rounded-r-md w-56 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] py-0 relative before:block before:w-px before:h-8 before:bg-slate-200 before:absolute before:left-0 before:inset-y-0 before:my-auto before:dark:bg-darkmode-400">
                    <div className="flex items-center justify-center">
                      <Link to={`/edit-lead-source/${item.id}`}>
