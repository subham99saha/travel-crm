...
import { fetchAllTransport, deleteTransport } from "./vendorTransportSlice";
...
const Main = ()=>{
    ...
    const { data } = useAppSelector((state) => state.vendorTransport);
  ...
  const handleDelete = (id: any) => {
    dispatch(deleteTransport(deleteValueRef.current));
  ...
  return (
    <>
      <h2 className="mt-10 text-lg font-medium intro-y">Manage Transport (Vendor)</h2>
      <div className="grid grid-cols-12 gap-6 mt-5">
        <div className="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap">
        <Link to="/add-vendor-transport">
            ...
            <Table.Tbody>
              {_.take(data, data.length).map((item, Key) => (
                <Table.Tr key={Key} className="intro-x">
                    ...
                    <div className="flex">
                      <div className="w-20 h-20 image-fit zoom-in">
                        ...
                        <Tippy
                          ...
                          src={`${server}transport/${item.mainImg}`}
                          content={item.transportName}
                           onClick={()=>{
                             navigate(`/edit-vendor-transport/${item.id}`)
                           }}
                        />
                  ...
                  <Table.Td className="first:rounded-l-md last:rounded-r-md bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]">
                    {item.priceSeasonAC ? `AC: ${item.priceSeasonAC}`: '--'}<br />
                    {item.priceSeasonAC ? `Non AC: ${item.priceSeasonNonAC}`: '--'}
                  </Table.Td>
                  <Table.Td className="first:rounded-l-md last:rounded-r-md bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]">
                    {item.priceOffSeasonAC ? `AC: ${item.priceOffSeasonAC}`: '--'}<br />
                    {item.priceOffSeasonNonAC ? `Non AC: ${item.priceOffSeasonNonAC}`: '--'}
                  </Table.Td>
                  <Table.Td className="first:rounded-l-md last:rounded-r-md bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]">
                  ...
                  <Table.Td className="first:rounded-l-md last:rounded-r-md w-56 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] py-0 relative before:block before:w-px before:h-8 before:bg-slate-200 before:absolute before:left-0 before:inset-y-0 before:my-auto before:dark:bg-darkmode-400">
                    <div className="flex items-center justify-center">
                      <Link to={`/edit-vendor-transport/${item.id}`}>
