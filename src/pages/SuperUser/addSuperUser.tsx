...
import { createSuperUser } from "./SuperUserSlice";

import City from "../../components/CustomComponents/City";
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
        ...
        sEmail: event.target[2].value,
        sPass: event.target[3].value,
        isActive: event.target[4].value,
        menuId: event.target[9].value,
      };
      // console.log(event.target);
      // console.log("My Value: ",event.target[9].value);
      dispatch(createSuperUser(formData));
      reset({
        sName: "",
        isActive: "Y",
      });
      event.target[4].value = "Y";
