...
import { getItineraryProduct, updateItineraryProduct } from "./ItineraryProductSlice";

const editItineraryProduct = ()=>{
  const params = useParams();
  const dispatch = useAppDispatch();
  const { data } = useAppSelector((state) => state.itineraryProduct); 

  useEffect(()=>{
    dispatch(getItineraryProduct(params.id));
  },[])
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
       ...
       dispatch(updateItineraryProduct(formData));
                      ...
                      <div className="mt-3 input-form">
                        ...
                        <FormSelect 
                          ...
                          aria-label="Default select example">
                                <option value="Product" selected={(data.type==='Product')?true:false}>Product</option>
                                <option value="Service" selected={(data.type==='Service')?true:false}>Service</option>
                                <option value="Project" selected={(data.type==='Project')?true:false}>Project</option>
                        </FormSelect>
                      </div>
                      <div className="mt-3 input-form">
                        <FormLabel
...
export default editItineraryProduct;