...
import { createCruise } from "./cruiseSlice";
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
        ...
        departureTime: `${event.target[4].value}:${event.target[5].value}`,
        arrivalTime: `${event.target[6].value}:${event.target[7].value}`,
        isActive: event.target[8].value
       } ;
       console.log(formData);
       dispatch(createCruise(formData));
       reset({
        cruiseNo: "",
        departure: "",
        destination: "",
