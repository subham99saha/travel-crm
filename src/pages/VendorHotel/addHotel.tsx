...
import { createVendorHotel } from "./vendorHotelSlice";
...
const addHotel = ()=>{
  ...
  const [isCheckedEP, setisCheckedEP] = useState(true);
  const [isCheckedAP, setisCheckedAP] = useState(true);
  const [isCheckedMAP, setisCheckedMAP] = useState(true);
  const [isCheckedCP, setisCheckedCP] = useState(true);  
  ...
  useEffect(()=>{
    ...
    if (userType === "V") {
      setVendor(JSON.parse(localStorage.getItem('userInfo'))['vendorData']['id'])
  }
  },[]);
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    const res = axios.post(`${api.API_URL}${api.API_ENDPOINT.VENDOR_HOTEL}`, sendData, {}).then((result)=>{
      reset();
  ...
  return (
    <>
      ...
      <div className="flex items-center mt-8 intro-y">
        <h2 className="mr-auto text-lg font-medium">Add Hotel Details</h2>
