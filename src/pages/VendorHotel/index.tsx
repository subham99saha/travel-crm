...
const Main = ()=>{
  ...
  return (
    <>
      <h2 className="mt-10 text-lg font-medium intro-y">Manage Hotel (Vendor)</h2>
      <div className="grid grid-cols-12 gap-6 mt-5">
        ...
        {_.take(data, data.length).map((item, Key) => (
          ...
          >
             <div className="box">
              <div className="p-5">
                <div className="h-40 overflow-hidden rounded-md 2xl:h-56 image-fit before:block before:absolute before:opacity-30 before:w-full before:h-16 before:bottom-0 before:left-0 before:z-10 before:bg-black before:to-black/10">
                  <img
                  ...
                  <div className="absolute bottom-0 z-10 px-5 py-2 text-white">
                ...
                <div className="mt-5 text-slate-600 dark:text-slate-500">
                  ...
                  <div className="flex items-center">
                    <Lucide icon="Link" className="w-4 h-4 mr-2" /> Contact Number: {item.contactNumber}
                  </div>
                  }
                  <div className="flex items-center mt-2">
                    <Lucide icon="Layers" className="w-4 h-4 mr-2" /> Check-in Time: {item.checkInTimeH}:{(item.checkInTimeM!=''?item.checkInTimeM:'00')}
                  </div>
                  <div className="flex items-center mt-2">
                    <Lucide icon="Layers" className="w-4 h-4 mr-2" /> Check-out Time: {item.checkOutTimeH}:{(item.checkOutTimeM!=''?item.checkOutTimeM:'00')}
                  </div>
                  <div 
                  className={clsx([
              ...
              <Link to={`/view-vendor-hotel/${item.id}`} className="flex items-center mr-auto text-primary" href="#">
                  <Lucide icon="Eye" className="w-4 h-4 mr-1" /> Preview
                </Link>
                <Link to={`/edit-vendor-hotel/${item.id}`} className="flex items-center mr-3" href="#">
