...
import {
  ...
  const uploadImageToRepo = ()=>{
    ...
    const deleteSelected = () => {
      // dispatch(deleteImage({ files: selected, folder: repoFor }))
      const URL = `${api.API_URL}${api.API_ENDPOINT.IMAGE_REPO}`;
      axios.post(`${URL}/delete-image?folder=${repoFor}`, { files: selected, folder: repoFor }, {
        headers: {},
      }).catch(err => {
        console.log({err})
      });
      setDeleteStatus(selected.length + ' image(s) deleted') 
      setSelected([])
      setReload(reload + 1)      
    }
    
    // const repoImages = useAppSelector((state) => state.itinerary);
    ...
    useEffect(() => {
      ...
      axios.post(`${URL}/view-gallery/?folder=${repoFor}`, {}, {
        ...
        setRepoImages(result.data.files)
    ...
    const uploadRepoImage = async () => {
      if (Object.keys(repoObj).length != 0) {
        ...
        // console.log({repoObj})
        formData.append('folder', 'uploads/hotel');
        for (var [key, value] of formData.entries()){
          console.log(key,value);
        }
        // let x = await dispatch(uploadGalleryImage(formData,'uploads/hotel'))
        const URL = `${api.API_URL}${api.API_ENDPOINT.IMAGE_REPO}`;
        axios.post(`${URL}/upload-multi-image/${repoFor}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          },
        }).then((result)=>{
          console.log(result.data.filenames)
          setUploadStatus(result.data.filenames.length + ' image(s) uploaded succesfully.')
          setReload(reload + 1)
        }).catch(err => {
          console.log({err})
        });
                  ...
                  <div className="p-5">
                    <Preview>
                      ...
                      <div className="input-form">                        
                        ...
                        <Button variant="primary" type="submit" className="mt-5 w-24 mr-1" onClick={() => uploadRepoImage()}>
                            Upload
                        </Button>
            ...
            <div className="col-span-12 intro-y lg:col-span-12">
                <div className="intro-y box">                
                    ...
                    <div className="grid grid-cols-12 gap-6 p-5">
                        {/* {repoImages.data.files?.map((file,Key) => { */}
                        {repoImages.map((file,Key) => {
                            var border = ''
                            if (selected.includes(file)) {
                              border = '5px dashed white'
                            }
                            return <div key={Key} className="col-span-12 lg:col-span-3">
                              <div className="rounded " onClick={() => {select(file)}} style={{height:'200px', backgroundImage:`url(${api.API_URL}images/${repoFor}${file})`, backgroundPosition:'center', backgroundSize:'cover', border }}>
  ...
  export default uploadImageToRepo;