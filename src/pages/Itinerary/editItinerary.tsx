...
const edittinerary = () => {
  ...
  const runVal = (payload) => {
    ...
    if (errCount === 0) {
      ...
      setSuccessMsg('Data Updated Succesfully.')
    } else {  
      setErrors(err)
      console.log({errors})
    ...
    <div>
      ...
      <div className="grid grid-cols-12 gap-6 mt-5">
        ...
        { dayDetails.map((item,idx) => {
          ...
          <div className="flex flex-col items-center p-5 border-b sm:flex-row border-slate-200/60 dark:border-darkmode-400">
            <h2 className="mr-auto text-base font-medium">
              Edit Itinerary - Day {idx+1} Details
            </h2>
          </div>
          <div className="p-5">
                  ...
                  <DayPlan plan={item.plan} locations={item.locations} city={city} fromID={Number(item.fromId)} toID={Number(item.toId)} season={season} handleChange={(field,val) => handleDayDetails(idx+1,field,val)} type='edit' />
                  </div>
            </div>
        ...
        <div className="col-span-12 intro-y box lg:col-span-12">
          <div className="flex flex-col items-center p-5 border-b sm:flex-row border-slate-200/60 dark:border-darkmode-400">
            <h2 className="mr-auto text-base font-medium">
              Edit Itinerary - Inclusions and Exclusions
            </h2>
          </div>
          <div className="p-5">
            <div className="mb-5">
          ...
          <Slideover.Panel>
            ...
            {(hotel.data.length != 0) ? _.take(hotel.data, hotel.data.length).map((item, Key) => {
              ...
              return <div key={item.id} className="m-1 p-2 " onClick={() => {
                handleDayArrayDetails('hotel',item.id,item.hotelName,item.minRoomRentSeasonAC,2);                
              }}>
