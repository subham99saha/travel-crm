...
import { getVendorTrain, updateVendorTrain } from "./vendorTrainSlice";
...
const editTrain = ()=>{
  ...
  const imgaePath = `${server}images/train/`;
  const dispatch = useAppDispatch();
  const { data } = useAppSelector((state) => state.vendorTrain);
  ...
  useEffect(()=>{
    dispatch(getVendorTrain(params.id))
  },[]);
...
useEffect(()=>{
    reset(data);
    setTrainNo(data.trainNo);
    setVendor(data.createdBy);
    setSelectDptCity(String(data.departureCityId));
    setSelectDstCity(String(data.destinationCityId));
    setisActive(data.isActive);
    const dptTime = _.split(data.departureTime, ':');
    const arrTime = _.split(data.arrivalTime, ':');
    console.log(arrTime);
    setDepartureTimeH(dptTime[0]);
    setDepartureTimeM(dptTime[1]);
    setArrivalTimeH(arrTime[0]);
    setArrivalTimeM(arrTime[1]);
    console.log("Test", arrivalTimeH, arrivalTimeM);
...
//console.log(data);
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let sendData = {
       ...
       dispatch(updateVendorTrain(sendData));
  ...
  const handleClick = ()=>{
    ...
    if(fileName){
      ...
      const res = axios.post(`${server}super-train-master/update-img/`, formdata, {
        headers: {
