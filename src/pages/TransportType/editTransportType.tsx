...
import { getTransportType, updateTransportType } from "./TransportTypeSlice";
...
const editCurrency = ()=>{
  ...
  useEffect(()=>{
    dispatch(getTransportType(params.id));
  },[])
  
  const schema = yup
    .object({
      transportType: yup.string().required().min(2),
    })
    .required();
  ...
  const onSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (!result) {
      ...
      let formData = {
        ...
        transportType: event.target[1].value,
        isActive: event.target[2].value,
        id: params.id
       } ;
       
       dispatch(updateTransportType(formData));
  ...
  return (
    <>
      <div className="flex items-center mt-8 intro-y">
        <h2 className="mr-auto text-lg font-medium">Manage Transport Type</h2>
      </div>
                    ...
                    <form className="validate-form" onSubmit={onSubmit}>
                      ...
                      <FormInput
                        ...
                        <FormInput
                          ...
                          name="transportType"
                          className={clsx({
                            "border-danger": errors.transportType,
                          })}
                          placeholder="Transport Type"
                        />
                        {errors.transportType && (
                          <div className="mt-2 text-danger">
                            {typeof errors.transportType.message === "string" &&
