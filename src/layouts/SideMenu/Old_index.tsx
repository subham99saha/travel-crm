...
import TopBar from "../../components/TopBar/index0";
...
function Main() {
  ...
  useEffect(() => {
    if (localStorage.getItem("userInfo") === null) {
      ...
      let level = JSON.parse(localStorage.getItem("userInfo"))["resData"][
        "userType"
      ];
      let mail = JSON.parse(localStorage.getItem("userInfo"))["resData"][
        "sEmail"
      ];
      ...
      if (level === "V") {
        setVendorType(
          JSON.parse(localStorage.getItem("userInfo"))["vendorData"]["vType"]
        );
  ...
  return (
    <div className="py-5 md:py-0">
      ...
      <div className="flex overflow-hidden">
        {/* BEGIN: Side Menu */}
        <nav className="w-[105px] xl:w-[260px] px-5 pb-16 overflow-x-hidden z-50 pt-40 -mt-4 hidden md:block bg-orange-200 sidebar_mebu">
          <ul>
            {/* BEGIN: First Child */}
            {formattedMenu.map((menu, menuKey) =>
              menu == "divider" ? (
                ...
                <li key={menuKey}>
                  ...
                  ) : menu.userType.toLowerCase() === vendorType ? (
                    <>
                      <Menu
                        className={clsx({
                          // Animation
        ...
        <div
          className={clsx([
            "max-w-full md:max-w-none rounded-[30px] md:rounded-none px-4 md:px-[22px] min-w-0 min-h-screen bg-[#ffead2] flex-1 md:pt-28 pb-10 mt-5 md:mt-1 relative dark:bg-darkmode-700",
